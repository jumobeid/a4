...
class PracticeController extends Controller {
    ...
    */
    public function practice17() {
        ...
        $tasks = Task::orderBy('id', 'desc')->get()->take(5);
    ...
    */
    public function practice10() {
        ...
        $tasks = Task::where('title','=' , 'Just a test')->get();
        //dd($tasks);
        if(count($tasks)==0) {
            ...
            foreach($tasks as $key => $task) {
                ...
                $task->title = 'Morning Assesment';
                $task->due_date = '2025-05-28';
    ...
    */
    public function practice9() {
        ...
        # First get a book to update
        $task = Task::where('title', 'LIKE', '%Milking%')->first();

        if(!$task) {
            dump("Task not found, can't update.");
        }
        else {

            # Change some properties
            $task->title = 'Milking the cows in the morning';
            $task->due_date = '2025-05-28';
    ...
    */
    public function practice8() {
        ...
        $task = new Task();
        $tasks = $task->where('title', 'LIKE', '%Milking%')
        ->where('due_date', '>=', 2017)
    ...
    * Lecture 11) Constraint chaining
    */
    public function practice88() {

        $task = new Task();
        $tasks = $task->where('title', 'LIKE', '%Milking%')
        ->orWhere('due_date', '>=', 2017)
        ->orderBy('created_at','desc')
        ->get();
    ...
    */
    public function practice6() {
        ...
        $newTask->title = "Milking the cows";
  		  $newTask->priority = "Hi";
  		  $newTask->start_date ="2017-07-20";
  		  $newTask->due_date ="2017-07-21";
  		  $newTask->complete_percent ="0";
        $newTask->location_id =6;
  		  $newTask->notes ="This is to test miling the cows";
        $newTask->status="Not completed";
        $newTask->done_overdue='No';

        $newTask->save();

        dump($newTask->toArray());

    }


    /**
    * Example for Clayton//ASK KATE
    */
